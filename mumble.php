<?php
	...
	$mumbleurl = "mumble://mumble.dk.steamlug.org/";
	...
	global $statusString;
		...
		$statusString .= "\n<ul>\n";
			if (is_array($value) && count($value) > 0 && isset($value['name']))
			{
					...
					if ( is_array($value['users']) && count($value['users']) > 0 && isset($value['name']))
					{
						Users($value['users']);
					}
