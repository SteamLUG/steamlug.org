...
// and should follow the same LICENSE as that code. We can probably trim it in
// future, but only admins hit it, so blergh
...
// SteamLUG hook code for all the tweet boxes.
(function () {
    "use strict";
    ...
    var lengther = {
        ...
        /*
		* Collect all the inputs, append a label for the character count,
		* resize the input so we still look good for users without js
         */
        init: function () {
            ...
            for (i = 0; i < nodes.length; i += 1) {
                ...
                var tweet = nodes[i];
				tweet.classList.remove('col-xs-11');
				tweet.classList.add('col-xs-10');

				var counter = document.createElement( 'label' );
				counter.classList.add('col-xs-1');
				counter.classList.add('control-label');

				tweet.parentNode.appendChild( counter );
				this.update_tweet( tweet );

				tweet.addEventListener("keyup", this.update_tweet );

            }
        },

		update_tweet: function( tweet ) {

			if ( tweet.target )
				tweet = tweet.target;

			length = twttr.txt.getTweetLength( tweet.value );
			var send = tweet.parentNode.firstChild;
			var counter = tweet.parentNode.lastChild;

			counter.innerHTML = length;
			if (length > 140) {
				tweet.classList.add('warn');
				tweet.classList.remove('caution');
				counter.classList.add('warn');
				counter.classList.remove('caution');
				tweet.parentNode.firstChild.disabled = 'disabled';
			} else if ( length > 130 ) {
				tweet.classList.add('caution');
				tweet.classList.remove('warn');
				counter.classList.add('caution');
				counter.classList.remove('warn');
				tweet.parentNode.firstChild.disabled = '';
			} else {
				tweet.classList.remove('caution');
				tweet.classList.remove('warn');
				counter.classList.remove('caution');
        ...
        /*
         * A callback that fires when the user lifts a finger in the tweet boxes
         */
